...
final class TeamsWebView: WKWebView, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    ...
    override init(frame: CGRect, configuration: WKWebViewConfiguration) {
        ...
        if responds(to: #selector(setter: WKWebView.allowsPictureInPictureMediaPlayback)) {
            allowsPictureInPictureMediaPlayback = true
        }

        // UA de Safari macOS para que Teams no fuerce la app móvil
        customUserAgent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 13_5) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Safari/605.1.15"

        // Inyección de scripts para notificaciones/heurísticas de título
        ...
        // Para que el alert/confirm/prompt funcionen nativo
        UNUserNotificationCenter.current().delegate = nil
    }

    required init?(coder: NSCoder) { fatalError("init(coder:) has not been implemented") }

    // MARK: - WKScriptMessageHandler (puente JS → nativo)
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if let dict = message.body as? [String: Any] {
            ...
            case "title":
                // Heurística: títulos con contador/keywords
    ...
    // MARK: - Navegación / ventanas / diálogos

    // Abrir esquemas externos fuera (mailto:, tel:, etc.) y bloquear msteams://
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction,
                 decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {

        if let url = navigationAction.request.url {
            if let scheme = url.scheme?.lowercased(), !scheme.hasPrefix("http") {
                // Bloquea intentos de ir a la app nativa
                if scheme.contains("msteams") {
                    decisionHandler(.cancel); return
                }
                // Para otros esquemas, abre fuera
                UIApplication.shared.open(url, options: [:], completionHandler: nil)
                decisionHandler(.cancel); return
            }
        }

        // target=_blank → cargar en el mismo webview
    ...
    // Maneja window.open / target=_blank
    ...
    func webView(_ webView: WKWebView, runJavaScriptAlertPanelWithMessage message: String,
        ...
        presentAlert(title: nil, message: message, actions: [
            UIAlertAction(title: "OK", style: .default, handler: { _ in completionHandler() })
        ])
    }
    ...
    (function() {
      ...
      // Observa cambios de título para contadores y eventos
      try {
        const target = document.querySelector('title') || document.head;
        const obs = new MutationObserver(() => {
          try {
            const t = document.title || '';
            NATIVE && NATIVE.postMessage({ type: 'title', value: t });
          } catch(e) {}
        });
        obs.observe(target, { subtree: true, characterData: true, childList: true });
      } catch (e) {}

      // "Monkey patch" suave de Notification para avisar nativo (sin romper la web)
      try {
        if (window.Notification) {
          const _N = window.Notification;
          const wrapper = function(title, opts) {
            try {
              NATIVE && NATIVE.postMessage({ type: 'notification', title: String(title || 'Teams'), body: (opts && opts.body) || '' });
            } catch(e) {}
            try { return new _N(title, opts); } catch(e) { return undefined; }
          };
          wrapper.requestPermission = function(cb) {
            try { cb && cb('granted'); } catch(e) {}
